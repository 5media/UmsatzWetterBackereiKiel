...
for category in categories:
    ...
    conf = model.conf_int().iloc[1:].values.tolist()  # Konfidenzintervalle als Liste von Listen
...
# Erstellen eines DataFrames für die Plot-Daten
coef_df = pd.DataFrame(coefficients, index=categories, columns=independent_vars)

# Erstellen eines DataFrames für Konfidenzintervalle
conf_df = pd.DataFrame(index=categories, columns=pd.MultiIndex.from_product([independent_vars, ['lower', 'upper']]))
for i, var in enumerate(independent_vars):
    conf_df[var] = [(conf[i][0], conf[i][1]) for conf in conf_intervals]

# Beispiel für die Ausgabe des DataFrames
print(conf_df)
...
for i, var in enumerate(independent_vars):
    plt.subplot(1, 2, i+1)
    sns.barplot(x=coef_df.index, y=coef_df[var])
    plt.errorbar(x=coef_df.index, y=coef_df[var], 
                 yerr=[(coef_df[var] - conf_df[(var, 'lower')].apply(lambda x: x[0])).values, 
                       (conf_df[(var, 'upper')].apply(lambda x: x[1]) - coef_df[var]).values], 
